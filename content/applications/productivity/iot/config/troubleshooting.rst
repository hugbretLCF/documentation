...
to proprietary RJ45 cable is required for the scale to function with Odoo's :abbr:`IoT (Internet of
Things)` box.
...
cable outside of this Mettler cable will work for this configuration. Using a serial-only cable
attached to a serial to :abbr:`USB (Universal Serial Bus)` adapter will **not** be effective.
...
hold the :guilabel:`>T<` button for eight seconds, or until :guilabel:`CONF` appears.

Next, press :guilabel:`>T<` until :guilabel:`GRP 3` appears, then press :guilabel:`>0<` to confirm.
...
Continue to press :guilabel:`>0<` until :guilabel:`GRP 4` appears. Do **not** make any other changes
...
Once :guilabel:`EXIT` appears, press :guilabel:`>0<`. Following this, press :guilabel:`>0<` again to
:guilabel:`SAVE`. Now the scale will restart.
